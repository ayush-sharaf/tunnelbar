...
![Tunnelnest manager window](docs/screenshot.png)
...
- **Persistent** — connections are saved to
  `~/Library/Application Support/Tunnelnest/connections.json`; per-connection
  logs go to `…/Tunnelnest/logs/<id>.log`.

## Install

**Requirements:** macOS 14+ (Apple Silicon). Tunnelnest isn't notarized by Apple
...
Grab `Tunnelnest-x.y.dmg` from
[Releases](https://github.com/ayush-sharaf/tunnelnest/releases/latest), drag
**Tunnelnest** into **Applications**, then clear the quarantine flag once:
...
cd tunnelnest
./build.sh          # compiles and produces Tunnelnest.app
open Tunnelnest.app
...
disk under `~/Library/Application Support/Tunnelnest/` (a per-user directory).
...
machine; Tunnelnest only spawns the commands you give it.

## Project layout

| File | Purpose |
|------|---------|
| `Sources/Tunnelnest/main.swift` | Entry point; starts as an accessory (menu-bar) app |
